// src/components/3d/particles/attractor.js
import * as THREE from 'three';

export function createAttractor(length, start) {
  const positions = [];
  
  // Random starting point on sphere if no start provided
  const p = start ? start : [
    (Math.random() - 0.5) * 2,
    (Math.random() - 0.5) * 2, 
    (Math.random() - 0.5) * 2
 ...
 * Each returns [dx, dy, dz] derivatives for the given point and timestep
 */
...
export function aizawaAttractor([x, y, z], timestep) {
  ...
  const dz = (c + a * z - (z * z * z) / 3 - x * x + f * z * (x * x * x)) * timestep;
...
// Array of all available attractors for random selection

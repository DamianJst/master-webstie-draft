...
function AnimatedCamera(props) {
  const group = useRef();
  const { nodes, materials, animations } = useGLTF('/models/portfolio_website_camera.glb');
  const { actions } = useAnimations(animations, group);
  const { registerCameraAnimations } = useNavigation();
  const isRegistered = useRef(false);
  ...
  useEffect(() => {
    ...
    if (actions && Object.keys(actions).length > 0 && !isRegistered.current) {
      ...
      if (mixer) {
        console.log('AnimatedCamera: Registering camera animations:', actionNames);
        registerCameraAnimations(actions, mixer);
        isRegistered.current = true;
        console.log('AnimatedCamera: Camera animations registered successfully');
      } else {
        console.warn('AnimatedCamera: No mixer found for actions');
      }
    } else if (!actions) {
  ...
  }, [actions, registerCameraAnimations, animations]);
